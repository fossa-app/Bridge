...
type CompanyClient(transport: IHttpTransport) =
    member _.GetCompanyAsync() : Task<CompanyRetrievalModel> =
        transport.GetAsync<CompanyRetrievalModel>(composeRelativeUrl [ Endpoints.Company ] [])

    member _.CreateCompanyAsync(model: CompanyModificationModel) : Task =
        transport.PostAsync<CompanyModificationModel>(composeRelativeUrl [ Endpoints.Company ] [], model)

    member _.UpdateCompanyAsync(model: CompanyModificationModel) : Task =
        transport.PutAsync<CompanyModificationModel>(composeRelativeUrl [ Endpoints.Company ] [], model)

    member _.DeleteCompanyAsync() : Task =
        transport.DeleteAsync(composeRelativeUrl [ Endpoints.Company ] [])
